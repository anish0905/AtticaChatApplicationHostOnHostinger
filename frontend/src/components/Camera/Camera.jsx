    ...
    <div className='font-semibold text-xl text-white w-50'>
      ...
      {showOptions && (
        <div className='flex justify-center content-center items-center gap-10' >
          <FaCamera  onClick={() => handleOptionClick('frontend')} className='text-3xl'/>
          <FaCameraRotate onClick={() => handleOptionClick('back')} className='text-4xl'/>
        </div>
      )}
      {selectedCamera === 'frontend' && <FrontendCamera onCapture={onCapture} onClose={onClose} recipient={recipient} loggedInUserId={loggedInUserId} admin={admin} />}
      {selectedCamera === 'back' && <BackCamera onCapture={onCapture} onClose={onClose} recipient={recipient} loggedInUserId={loggedInUserId} admin={admin} />}
    </div>
  );
};
