...
const BlockAccess = () => {
    ...
    const handleBlock = async (userId) => {
        try {
            ...
            setUsers(prevUsers => prevUsers.map(user => user._id === userId ? { ...user, access: false } : user));
            Swal.fire({
                title: 'Blocked!',
    ...
    const handleUnblock = async (userId) => {
        try {
            ...
            await axios.put(`${BASE_URL}/api/${endpoint}${userId}`);
            setUsers(prevUsers => prevUsers.map(user => user._id === userId ? { ...user, access: true } : user));
            Swal.fire({
                title: 'Unblocked!',
                        ...
                        {filteredUsers.map(user => (
                            <li key={user._id} className='flex items-center justify-between gap-5 my-4'>
                                ...
                                <div className='flex items-center justify-between gap-2 text-white text-sm'>
                                    {user.access ? (
                                        <button className='bg-red-700 px-2 py-1 rounded-md shadow-md' onClick={() => handleBlock(user._id)}>Block</button>
                                    ) : (
