  ...
  return (
    <div className='flex flex-row lg:flex-col h-[80px] lg:h-screen w-full lg:w-[100px] left-0 bg-[#5443c3] border-b lg:border-r shadow-md justify-between items-center py-[10px] lg:py-[20px] text-gray-500 text-2xl md:text-3xl'>
      ...
      <div className="flex flex-row lg:flex-col gap-[10px] sm:gap-[10px] md:gap-[10px] lg:gap-[40px] relative">
        <div 
          onClick={handleChat} 
          className={`group relative flex items-center rounded-full p-3 md:p-5 ${isActive("/HrToHrChat") || isActive("/AccountToAccountChat") || isActive("/SoftwareToSoftwareChat") || isActive("/bouncerChat") || isActive("/DigitalMarketingChatToDigitalMarketing") || isActive("/monitoringTeamChat") || isActive("/VirtualTeamToVirtualTeam") || isActive("/SecurityChat") ? "bg-blue-500 text-white" : "bg-[#fffefd]"}`}
        >
          <BsChatSquareDots />
        ...
        <div 
          ...
          className={`group relative flex items-center rounded-full p-3 md:p-5 ${isActive("/HrToAdminChat") || isActive("/AccountToAdminChat") || isActive("/SoftwareToAdminChat") || isActive("/BouncerToAdminChat") || isActive("/DigitalMarketingToAdminChat") || isActive("/MonitoringTeamToAdminChat") || isActive("/VirtualTeamToAdminChat") || isActive("/SecurityToAdminChat") ? "bg-blue-500 text-white" : "bg-[#fffefd]"}`}
        >
          <RiContactsLine/>
          <span className="absolute top-full lg:top-auto lg:left-full ml-2 lg:ml-0 lg:mt-2 lg:mb-0 whitespace-nowrap bg-black text-white text-xs md:text-sm rounded py-1 px-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            Admin
      ...
      <p className='text-white text-base lg:text-xl items-center hidden lg:flex'>{userDetails?.name}</p>
      </div>


      <div onClick={handleLogout} className="group relative flex items-center bg-[#fffefd] hover:bg-blue-500 rounded-full p-3 md:p-5 ">
        <BiLogOut />
        <span className="absolute top-full lg:top-auto lg:left-full ml-2 lg:ml-0 lg:mt-2 lg:mb-0 whitespace-nowrap bg-black text-white text-xs md:text-sm rounded py-1 px-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          Logout
        </span>
