...
const MonitoringTeamSideBar = () => {
  const navigate = useNavigate();
  const location = useLocation();
  
  const handleChat = () => {
    navigate("/monitoringTeamChat"); // Adjust route for monitoring team chat
  };

  const handleAdmin = () => {
    navigate("/MonitoringTeamToAdminChat"); // Adjust route for admin chat
  };
  ...
  return (
    <div className='flex flex-row lg:flex-col h-[80px] lg:h-screen w-full lg:w-[140px] left-0 bg-[#5443c3] border-b lg:border-r shadow-md justify-between items-center py-[10px] lg:py-[20px] text-gray-500 text-2xl md:text-3xl'>
      ...
      <div className="flex flex-row lg:flex-col gap-[10px] sm:gap-[10px] md:gap-[10px] lg:gap-[40px] relative">
        <div 
          onClick={handleChat} 
          className={`group relative flex items-center rounded-full p-3 md:p-5 ${isActive("/monitoringTeamChat") ? "bg-blue-500 text-white" : "bg-[#fffefd]"}`}
        >
          <BsChatSquareDots />
        ...
        <Link 
          to="/MonitoringTeamToAdminChat" 
          className={`group relative flex items-center rounded-full p-3 md:p-5 ${isActive("/MonitoringTeamToAdminChat") ? "bg-blue-500 text-white" : "bg-[#fffefd]"}`}
        >
          <RiContactsLine />
          <span className="absolute top-full lg:top-auto lg:left-full ml-2 lg:ml-0 lg:mt-2 lg:mb-0 whitespace-nowrap bg-white text-black text-xs md:text-sm rounded py-1 px-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            Admin
...
export default MonitoringTeamSideBar;
