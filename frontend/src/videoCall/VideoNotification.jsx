...
const VideoNotification = () => {
    ...
    useEffect(() => {
        const fetchRequests = async () => {
            try {
                ...
                // Filter out requests where senderId is the same as userId (i.e., the user is calling themselves) and only keep 'pending' requests
                const filteredRequests = data.videoCallRequests
                ...
                newRequests.forEach(request => {
                    setTimeout(() => handleReject(request._id), 6000); // 60000 ms = 1 minute
                });
    ...
    return (
        <div className="fixed bottom-4 right-10 space-y-4 z-50">
            {requests.length === 0 ? (
                <p className="text-lg font-semibold text-gray-600">No pending calls</p>
            ) : (
