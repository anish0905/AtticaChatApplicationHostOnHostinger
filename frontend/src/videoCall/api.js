...
export const getVideoCallRequests = async (receiverId) => {
    try {
        const response = await axios.get(`${BASE_URL}/api/videoCall/video-call/requests/${receiverId}`);
...
export const acceptVideoCall = async (videoCallRequestId) => {
    try {
        const response = await axios.post(`${BASE_URL}/api/videoCall/video-call/accept/${videoCallRequestId}`);
        return response.data;
    } catch (error) {
...
export const rejectVideoCall = async (videoCallRequestId) => {
    try {
        const response = await axios.post(`${BASE_URL}/api/videoCall/video-call/reject/${videoCallRequestId}`);
        return response.data;
    } catch (error) {
